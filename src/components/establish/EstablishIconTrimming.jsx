...
const EstablishIconTrimming = (props) => {
    ...
    const handleModalClose = () => {
        ...
        props.setPreviewIcon(props.previewPrevIcon);
        props.setPreviewIcon(props.previewPrevIcon);
    ...
    const handleTrimmingComplete = () => {
        ...
        // トリミングした画像を設定
        props.setPreviewIcon(dataUrl);
        // 今回の画像を履歴として保存(次トリミングキャンセルした際に前のトリミング画像を表示。なければ前画像は空)
        props.setPreviewPrevIcon(dataUrl);
        // ズームと位置を初期化して閉じる。
        props.setIconCrop({x: 0, y: 0});
        props.setIconZoom(1);
        props.setOpen(false);
...
export default EstablishIconTrimming
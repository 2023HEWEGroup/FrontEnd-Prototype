...
const UseProduct = (props) => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                const response = await axios.get(`http://localhost:5000/client/product/getOrderUser/${props.user._id}?page=${1}&pageSize=${pageSize}`);
    ...
    useEffect(() => {
        ...
        try {
            const response = await axios.get(`http://localhost:5000/client/product/getOrderUser/${props.user._id}?page=${pageNumber + 1}&pageSize=${pageSize}`);
        ...
        <Slide in direction={props.direction}>
            <StyledUserProduct>
                <StyledTitle theme={theme}>
                    出品中の商品 ({props.user.products.length}件)
                </StyledTitle>
                <StyledProductTable>
                    {!isLoading ?

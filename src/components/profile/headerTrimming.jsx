...
const HeaderTrimming = (props) => {
    ...
    const handleModalClose = () => {
        ...
        props.setUploadHeader(props.uploadPrevHeader);
        props.setOriginalHeader(props.originalPrevHeader);
    ...
    const getCropSize = () => {
        return { width: imageDimensions.width, height: imageDimensions.width / 4 };
    };
    ...
    const handleTrimmingComplete = () => {
        ...
        // トリミングした画像を設定
        props.setUploadHeader(dataUrl);
        // 今回の画像を履歴として保存(次トリミングキャンセルした際に前のトリミング画像を表示。なければ前画像は空)
        props.setUploadPrevHeader(dataUrl);
        // ズームと位置を初期化して閉じる。
        props.setHeaderCrop({x: 0, y: 0});
        props.setHeaderZoom(1);
        props.setOpen(false);

...
const IconTrimming = (props) => {
    ...
    const handleModalClose = () => {
        ...
        props.setUploadIcon(props.uploadPrevIcon);
        props.setOriginalIcon(props.originalPrevIcon);
        props.setBinaryIcon(props.binaryPrevIcon);
        // ズームと位置を初期化して閉じる。
    ...
    const handleTrimmingComplete = () => {
        ...
        // トリミングした画像を設定
        props.setUploadIcon(dataUrl);
        // 今回の画像を履歴として保存(次トリミングキャンセルした際に前のトリミング画像を表示。なければ前画像は空)
        props.setUploadPrevIcon(dataUrl);
        // ズームと位置を初期化して閉じる。
        props.setIconCrop({x: 0, y: 0});
        props.setIconZoom(1);
        props.setOpen(false);
        ...
        canvas.toBlob((blob) => {
            props.setBinaryIcon(blob);
            // 履歴をバイナリ保存
            props.setBinaryPrevIcon(blob);
        })
    };

    const handleCropChange = (crop) => {
        props.setIconCrop(crop);
    }

    const handleZoomChange = (zoom) => {
        props.setIconZoom(zoom);
    }
    ...
    return (
        ...
        <Modal open={props.open}>
            ...
            <Cropper image={props.originalIcon} crop={props.iconCrop} zoom={props.iconZoom} minZoom={1} maxZoom={4} aspect={ASPECT_RATIO} onCropChange={handleCropChange}
